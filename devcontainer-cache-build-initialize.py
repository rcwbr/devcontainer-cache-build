...
def docker_destination_to_string(docker_destination_dict):
    ...
    """
    return ",".join(
        [f"{key}={value}" for key, value in docker_destination_dict.items()]
    )
...
def docker_destinations_to_string(docker_destination_list):
    ...
    return " ".join(
        [
            docker_destination_to_string(destination)
            for destination in docker_destination_list
        ]
    )
...
DEVCONTAINER_BUILD_ADDITIONAL_ARGS = dict_from_string(
    env.get("DEVCONTAINER_BUILD_ADDITIONAL_ARGS", "")
)
...
DEVCONTAINER_PUSH_IMAGE = env.get("DEVCONTAINER_PUSH_IMAGE", "false").lower() in [
    "true",
    "t",
    "yes",
    "y",
...
DEVCONTAINER_DEFAULT_BRANCH_NAME_SANITIZED = sanitize_ref(
...
if DOCKER_CONFIG_JSON is not None:
    try:
        ...
        if docker_config_filename.exists() and docker_config_filename.is_file():
            print(
                f"Docker config file {docker_config_filename} exists, will merge config content"
            )
            with open(docker_config_filename, "r") as docker_config_file:
                try:
                    docker_config_content = (
                        json.load(docker_config_file) | docker_config_content
                    )
                except ValueError:
                    print(
                        f"Invalid JSON content in {docker_config_filename}: {docker_config_file.read()}"
                    )
...
if DEVCONTAINER_PREBUILD_SCRIPT is not None:
    print(
        f"DEVCONTAINER_PREBUILD_SCRIPT {DEVCONTAINER_PREBUILD_SCRIPT} provided, running..."
    )
    prebuild_result = subprocess.run(
        [DEVCONTAINER_PREBUILD_SCRIPT], capture_output=True, text=True
    )
    print(
        f"DEVCONTAINER_PREBUILD_SCRIPT {DEVCONTAINER_PREBUILD_SCRIPT} result: {prebuild_result.stdout}"
    )
...
if INITIALIZE_BUILDER_NAME in [builder.name for builder in docker.buildx.list()]:
    ...
    print(
        f"Builder {INITIALIZE_BUILDER_NAME} found with status {docker.buildx.inspect(INITIALIZE_BUILDER_NAME).status}"
    )
else:
    print(f"No builder {INITIALIZE_BUILDER_NAME} found, creating:")
    initialize_builder = docker.buildx.create(
        bootstrap=True, driver="docker-container", name=INITIALIZE_BUILDER_NAME
    )
    print(
        f"Builder {INITIALIZE_BUILDER_NAME} created, status {initialize_builder.status}"
    )
...
devcontainer_host_env_vars = {
    ...
    and name.removeprefix(DEVCONTAINER_HOST_ENV_VAR_PREFIX)
    not in [
        "PATH",
        "HOME",
    ]
...
for var_name in ["USER", "UID", "USER_GID"]:
    if (
        f"{DEVCONTAINER_CACHE_BUILD_OVERRIDE_PREFIX}{var_name}"
        in devcontainer_host_env_vars
    ):
        value = devcontainer_host_env_vars[
            f"{DEVCONTAINER_CACHE_BUILD_OVERRIDE_PREFIX}{var_name}"
        ]
...
if DEVCONTAINER_DEFINITION_TYPE.lower() == "bake":
    ...
    bake_params = {
        "variables": {  # Bake variables are overridden by env vars
            ...
            "DEVCONTAINER_CACHE_FROMS": docker_destinations_to_string(
                DEVCONTAINER_CACHE_FROMS
            ),
            "DEVCONTAINER_CACHE_TOS": docker_destinations_to_string(
                DEVCONTAINER_CACHE_TOS
            ),
        ...
        if "output" in target:
            for pull_image in target["output"]:
                if "ref" in dict_from_string(pull_image):
                    ...
                    print(
                        f"Pulling image for {target_name} cache population: {image_name}"
                    )
...
if (
    env.get("CI", "false").lower() in ["true", "t", "yes", "y", "1"]
    and DEVCONTAINER_DEFINITION_TYPE.lower() == "bake"
):
    ...
    final_image_output_dicts = [
        dict_from_string(output) for output in final_image_config["output"]
    ]
    final_image_output = (
        [
            output_dict["name"]
    ...
    print(
        f"CI environment for bake build detected, writing Bake config and ref {final_image_output} for final layer {final_layer} to GITHUB_OUTPUT {GITHUB_OUTPUT}"
    )
...
if DEVCONTAINER_DEFINITION_TYPE.lower() == "build":
    ...
    docker.buildx.build(
        ...
        cache_to=DEVCONTAINER_CACHE_TOS[
            0
        ],  # Python-on-whales limitation of one cache-to
        context_path=DEVCONTAINER_CONTEXT,
        **DEVCONTAINER_BUILD_ADDITIONAL_ARGS,
    )
elif DEVCONTAINER_DEFINITION_TYPE.lower() == "bake":
    # Docker bake case
    print(json.dumps(docker.buildx.bake(print=True, **bake_params), indent=2))
    # Execute bake build
